...
export default {
  ...
  computed: {
    ...
    breadcrumbLinks() {
      return [
        {to: `/tenants/${this.clientId}/permission-types`, name: "Permission Types"},
        {to: `/tenants/${this.clientId}/permission-types/new`, name: "New"}
      ];
    },
    inputState() {
      return {
  ...
  methods: {
    ...
    async onSaveClick() {
      ...
      if (this.isFormValid) {
        ...
        try {
          await this.$store.dispatch("sharing/createPermissionType", {
            clientId: this.clientId,
            id: this.id,
            name: this.name,
            description: this.description
          });
          await this.$router.push(`/tenants/${this.clientId}/permission-types`);
        } catch (error) {
          this.errors.push({
            title: "Unknown error when creating the permission type.",
